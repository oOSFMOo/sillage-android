...
class MangaKeyer : Keyer<DomainManga> {
    override fun key(data: DomainManga, options: Options): String {
        ...
        } else if (needsCoverRepair(data.thumbnailUrl)) {
            "sillage-repair:${data.id};${data.coverLastModified}"
        } else {
...
class MangaCoverKeyer(
    ...
    override fun key(data: MangaCover, options: Options): String {
        ...
        } else if (needsCoverRepair(data.url)) {
            "sillage-repair:${data.mangaId};${data.lastModified}"
        } else {
