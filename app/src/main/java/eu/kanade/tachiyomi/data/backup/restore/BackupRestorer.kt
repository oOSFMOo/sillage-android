...
class BackupRestorer(
    ...
    private suspend fun restoreFromFile(uri: Uri, options: RestoreOptions) {
        val decodedBackup = BackupDecoder(context).decode(uri)
        val migration = if (options.libraryEntries && !isSync) {
            SillageBackupMigration.prepare(decodedBackup)
        } else {
            SillageBackupMigration.Prepared(decodedBackup, 0, false)
        }
        val backup = migration.backup
        ...
        coroutineScope {
            if (options.categories) {
                // Manga/category preferences resolve category names against these inserted rows.
        ...
        if (migration.isLegacyCatalogue) {
            ...
            Injekt.get<LibraryPreferences>().apply {
                ...
                updateCategories.set(emptySet())
                updateCategoriesExclude.set(emptySet())
