...
class MangaCoverFetcher(
    ...
    private val coverFile: (String?) -> File?,
    ...
    private var repairedDiskCacheKey: String? = null

    private val diskCacheKey: String
        get() = repairedDiskCacheKey ?: diskCacheKeyLazy.value
    ...
    override suspend fun fetch(): FetchResult {
        ...
        if (needsCoverRepair(url)) {
            val cachedCover = coverFile(url)
            if (cachedCover?.exists() == true && options.diskCachePolicy.readEnabled) {
                return fileLoader(cachedCover)
            }
            if (!options.networkCachePolicy.readEnabled) error("Cover unavailable offline")
            val source = sourceLazy.value ?: error("Install the source to load this cover")
            val repaired = ImportedCoverResolver.resolve(mangaId, source)
            url = repaired.url
            repairedDiskCacheKey = "${repaired.url};${repaired.lastModified}"
        }

        val coverUrl = url ?: error("No cover specified")
        return when (getResourceType(coverUrl)) {
            Type.File -> fileLoader(File(coverUrl.substringAfter("file://")))
            Type.URI -> fileUriLoader(coverUrl)
            Type.URL -> httpLoader()
    ...
    private suspend fun httpLoader(): FetchResult {
        ...
        val libraryCoverCacheFile = if (isLibraryManga) {
            coverFile(url) ?: error("No cover specified")
        } else {
            null
        }
    ...
    class MangaFactory(
        ...
        override fun create(data: Manga, options: Options, imageLoader: ImageLoader): Fetcher {
            return MangaCoverFetcher(
                ...
                options = options,
                coverFile = coverCache::getCoverFile,
